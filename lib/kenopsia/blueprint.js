...
const ROLES = [
  ...
  // Earned by talking to Tamem, not bought.
  { name: 'Tamem Whisperer', color: 0x9b8cf0 },
  { name: "Tamem's Teacher", color: 0x7c6fd6 },
  // A marker, not a gate. Tamem's allowlist decides who he reads and answers;
  // this role only exists so the server can see who that is. Granted by the
  // owner with /tamem allow — it is not for sale and carries no permissions.
  { name: 'Tamem Access', color: 0x5a5570 },
...
const COINS = {
  // 400 is 200 messages before the cap bites, so in practice it only binds for
  // the handful of people who genuinely talk all day. Raising it changes the
  // top of the server far more than the middle.
  message: { amount: 2, dailyCap: 400 },
